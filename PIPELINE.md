...
- **Land and housing** (a Wikidata pass for community land trusts and housing cooperatives, a seed list of Grounded Solutions Network US CLTs, Habitat for Humanity international country offices with in-place enrichment of the ~355 US Habitat affiliates already pulled from IRS data, and a construction-cooperatives source that combines Wikidata with seed entries for Mondragon, Italian construction co-ops, and French SCOPs).
...
- `ingest_land_and_housing.py` is the land-and-housing dispatcher. It runs four children: `ingest_land_trusts.py` (Wikidata SPARQL for community land trusts Q3278937 and housing cooperatives Q562166, ~440 new rows), `ingest_grounded_solutions.py` (the Grounded Solutions Network member-spotlight feed plus a curated seed list of US, UK, Canadian, and Belgian CLTs, ~40 rows), `ingest_habitat.py` (enriches the ~355 US Habitat for Humanity affiliates already in IRS_EO_BMF and adds ~65 international country offices), and `ingest_construction_coops.py` (Wikidata plus a seed list of Mondragon, Italian, French SCOP, and US/UK worker-owned construction firms, ~80 rows). Every row gets `legibility='formal'`.
...
Community land trusts, housing cooperatives, sweat-equity programs, and construction cooperatives are the second pillar of the framework after labor. The land-and-housing branch closes that gap. It runs in four sources stitched together by `ingest_land_and_housing.py`.

`ingest_land_trusts.py` queries Wikidata for two classes: community land trust (Q3278937) and housing cooperative (Q562166). The brief that scoped this pass cited the wrong QIDs for both, which is a small but instructive reminder that Wikidata IDs should be verified by running a label query before they are trusted. The correct IDs above were found by `wbsearchentities` and confirmed against instance counts. Every row inserted gets `legibility='formal'` and the Wikidata item URL as the evidence URL.

`ingest_grounded_solutions.py` is a two-part source. The Grounded Solutions Network's US CLT directory URL is a 404 as of 2026-04-24 and has no Wayback snapshot, so the primary path only caches the failure as evidence we tried. The fallback uses the WordPress REST API on `/wp-json/wp/v2/posts` with category filters for the Member Profile and Member Spotlight categories, which yields about eight blog posts about specific members. On top of that the script adds a curated seed list of 35 well-documented CLTs (Champlain Housing Trust, Dudley Neighbors, Oakland CLT, London CLT, and so on) so the branch has a real CLT footprint even though the global directory is not scrapeable today. Adding or editing names in the seed list is a normal code change rather than a data migration.

`ingest_habitat.py` does two things. First, it enriches the ~355 Habitat-for-Humanity rows already in the database from IRS_EO_BMF, tagging them with `registration_type='land_and_housing/sweat_equity_program'`, `framework_area='housing_land'`, `model_type='sweat_equity_program'`, and `legibility='formal'`. Second, it inserts ~65 international Habitat country offices (Habitat Brazil, Habitat Kenya, Habitat Philippines, and so on) from a hand-maintained seed list, with the country-office website as the evidence URL. The international list is kept in the script rather than scraped because habitat.org renders its where-we-work page client-side; a plain GET returns an HTML shell with no affiliate data.

`ingest_construction_coops.py` combines a Wikidata SPARQL query for cooperatives whose English label contains a construction-related token (construction, building, trades, and the German Bau) with a seed list of about 45 well-known construction and trades co-ops. The Mondragon Corporation (Spain), CMC Ravenna and CMB Carpi (Italy), Scopelec and UTB (France), South Mountain Company and Namaste Solar (US), and Co-operatives UK (UK) are all in the seed list. Construction co-ops are worker co-ops whose industry happens to be construction, so rows land in `framework_area='cooperatives'` with `registration_type='labor/construction_cooperative'`, not in the housing branch. The Wikidata half is noisy (about three quarters of the rows are German Baugenossenschaft entries that overlap with housing cooperatives) and dedup is left to `dedup_merge.py` as usual.

Across all four, about 630 new rows land in the database per run, plus another ~355 IRS Habitat rows get enriched in place. The branch is federation- and regional-tier only; individual housing co-op tenants, unregistered squats, and informal settlements are deliberately out of scope for v1 and would need human review before landing in the directory.
